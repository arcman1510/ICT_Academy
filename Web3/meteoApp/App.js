...
const getMockWeatherData = (city) => {
  ...
  const mockData = {
    ...
    windspeed: Math.floor(Math.random() * (50-0) + 0), // Venti tra 0 e 50
    condictions: ['Soleggiato', 'Piovoso', 'Nuvoloso', 'Nervoso'][Math.floor(Math.random() * 4)]
  };


  // Simuliamo una chiamata asincroma
  return new Promise((resolve) =>{
    setTimeout(() => {
      resolve(mockData);
    }, 2000); // Ritrado di 2 secondi per simulare una chiamata al servizio internet
  } )
}
  ...
  const WeatherInfo = ({data}) => {
    return (
      <View style = {styles.weatherContainer}>
        ...
        <Text style = {styles.descirption}>{data.condictions}</Text>
